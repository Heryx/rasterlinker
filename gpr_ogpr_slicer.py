...
# ---------------------------------------------------------------------------
# Helper: processa tutti i profili e ne estrae il dato float32
# ---------------------------------------------------------------------------
...
def _process_profiles(
    ...
    """Ritorna lista di (prof, ch_ref, ampl_3d)."""
    from .gpr_processing import apply_pipeline
...
def _build_grid_params(
    ...
    """Calcola bounding box, n_x/n_y, raggio e parametri anisotropia."""
...
def _interpolate_z_level(
    ...
    min_points: int,
    fill_nodata: bool,
    fill_nodata_max_distance: int,
    smooth_sigma: float,
) -> tuple[np.ndarray | None, int]:
    """
    Calcola la griglia interpolata per un livello z.
    Ritorna (grid, n_pts) oppure (None, 0) se nessun punto.
    """
    pts_e, pts_n, pts_a = [], [], []
...
# ---------------------------------------------------------------------------
# Anteprima: singola slice in RAM (nessun file scritto)
# ---------------------------------------------------------------------------
...
def compute_preview_slice(
    ...
    La slice viene calcolata al livello `z_center` con finestra
    [z_center - z_step/2, z_center + z_step/2].
    Gli stessi parametri e la stessa pipeline di slice_ogpr_to_tifs
    vengono applicati, inclusi IDW anisotropo, fill nodata e smoothing.

    Ritorna
    -------
    dict con:
        grid            : np.ndarray (n_y, n_x) float32
        x_min, y_min    : float  (angolo SW in unita' CRS)
        resolution      : float
        z_center        : float
        z_from, z_to    : float
        n_pts           : int  (punti dati prima dell'IDW)
        n_valid_cells   : int  (celle griglia non-NaN)
        n_total_cells   : int
        radius          : float (raggio effettivo usato)
        fill_pct        : float (percentuale celle valide)
    oppure None se nessun punto valido nel range z.
    """
...
def slice_ogpr_to_tifs(
    ...
    Utilizza gli helper interni _process_profiles, _build_grid_params
    e _interpolate_z_level condivisi con compute_preview_slice.
    """
    from .gpr_processing import DEFAULT_PIPELINE
    ...
    for iz, z_lev in enumerate(z_levels):
        ...
        # recupera n_ch usati dall'ultimo profilo
        n_ch_used = processed[-1][2].shape[2] if processed else 1
        n_valid   = int(np.count_nonzero(~np.isnan(grid)))
