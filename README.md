GeoSurvey Studio is a QGIS plugin developed to streamline the management and visualization of grouped raster datasets, particularly suited for GPR and grid-based survey workflows.

Key Features:
- Recursive detection of layer groups and subgroups
- Multi-group selection support
- Dynamic raster visibility control using dial and slider widgets
- Automatic cycling of rasters within selected groups
- Zoom-to-selected-groups functionality
- Real-time display of the currently visible raster name
- Aggregated raster listing across multiple selected groups
- Clean layer-tree integration without modifying original data

The plugin enhances structured raster navigation within complex QGIS projects, 
especially where multiple raster slices or time/depth grids must be inspected sequentially.

GeoSurvey Studio is designed to support non-invasive survey workflows and structured spatial analysis environments.

## How to Cite
If GeoSurvey Studio supports your work, please cite it as:

Guarino, G. (2026). GeoSurvey Studio (v1.0.1) [QGIS plugin]. GitHub. https://github.com/Heryx/rasterlinker

Suggested in-text citation:
- (Guarino, 2026)

Licensing note:
- Plugin source code: GPL-2.0-or-later
- Documentation/citation text: CC BY 4.0 (recommended for reuse with attribution)
